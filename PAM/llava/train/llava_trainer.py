...
def get_modality_length_grouped_indices(lengths, batch_size, world_size, generator=None):
    ...
    video_indices, video_lengths = zip(*[(i, -l) for i, l in enumerate(lengths) if l < 0])

    img_shuffle = [img_indices[i] for i in get_length_grouped_indices(img_lengths, batch_size, world_size, generator=None)]
    video_shuffle = [video_indices[i] for i in get_length_grouped_indices(video_lengths, batch_size, world_size, generator=None)]
    megabatch_size = world_size * batch_size
    img_megabatches = [img_shuffle[i : i + megabatch_size] for i in range(0, len(img_shuffle), megabatch_size)]
    video_megabatches = [video_shuffle[i : i + megabatch_size] for i in range(0, len(video_shuffle), megabatch_size)]

    last_img = img_megabatches[-1]
    last_video = video_megabatches[-1]
    additional_batch = last_img + last_video
    megabatches = img_megabatches[:-1] + video_megabatches[:-1]
    megabatch_indices = torch.randperm(len(megabatches), generator=generator)
    megabatches = [megabatches[i] for i in megabatch_indices]

    ### add the last megabatch. (It may be mix with image and video data)
